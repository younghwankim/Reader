...
#import "KWEPopoverParentView.h"
#import "UIBarButtonItem+KWEPopover.h"
#import "KWEPopoverContainerView.h"
#import "KWEWeakReference.h"
#import "UIView+KWEPopover.h"
#import "NSCondition+KWEPopover.h"
...
@interface KWEPopoverController()<KWETouchableViewDelegate>

@property (nonatomic, strong) KWEPopoverContainerView *containerView;
@property (nonatomic, strong) KWETouchableView *backgroundView;
...
@interface KWEPopoverController(Private)

- (UIView *)keyViewForView:(UIView *)theView;
- (void)updateBackgroundPassthroughViews;
- (CGRect)displayAreaForView:(UIView *)theView;
- (void)dismissPopoverAnimated:(BOOL)animated userInitiated:(BOOL)userInitiated completion:(KWEPopoverCompletionBlock)completion;
...
NSString * const KWEPopoverControllerWillShowNotification = @"KWEPopoverWillShowNotification";
NSString * const KWEPopoverControllerDidDismissNotification = @"KWEPopoverDidDismissNotification";

@implementation KWEPopoverController {
}

static KWEPopoverContainerViewProperties *defaultProperties = nil;
...
+ (NSUInteger)indexForActivePopover:(KWEPopoverController *)controller {
    @synchronized([KWEPopoverController class]) {
        NSMutableArray *array = [self activePopoverReferences];
        for (NSUInteger i = 0; i < array.count; ++i) {
            KWEWeakReference *value = [array objectAtIndex:i];
...
+ (void)pushActivePopover:(KWEPopoverController *)controller {
    @synchronized([KWEPopoverController class]) {
        id value = [KWEWeakReference kweweakReferenceWithObject:controller];
        [[self activePopoverReferences] addObject:value];
    }
}

+ (void)popActivePopover:(KWEPopoverController *)controller {
    @synchronized([KWEPopoverController class]) {
...
+ (NSArray *)visiblePopovers {
    @synchronized([KWEPopoverController class]) {
        ...
        for (KWEWeakReference *ref in popoverReferences) {
            if (ref.object != nil) {
...
+ (KWEPopoverContainerViewProperties *)defaultContainerViewProperties {
    ...
    KWEPopoverContainerViewProperties *props = [[KWEPopoverContainerViewProperties alloc] init];
...
- (id)init {
    if ((self = [super init])) {
        ...
        self.animationType = KWEPopoverAnimationTypeCrossFade;
...
- (void)presentPopoverFromBarButtonItem:(UIBarButtonItem *)item
    ...
    CGRect rect = [item kweFrameInView:v];
...
- (void)presentPopoverFromRect:(CGRect)rect
    ...
    if (!self.isPresenting && !self.isDismissing && ![self isPopoverVisible]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KWEPopoverControllerWillShowNotification object:self];
        ...
        _backgroundView = [[KWETouchableView alloc] initWithFrame:keyView.bounds];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyViewDidLayoutSubviewsNotification:) name:KWEViewDidLayoutSubviewsNotification object:keyView];

        KWEPopoverContainerViewProperties *props = self.containerViewProperties ? self.containerViewProperties : [[self class] defaultContainerViewProperties];
        KWEPopoverContainerView *containerView = [[KWEPopoverContainerView alloc] initWithSize:self.effectivePopoverContentSize anchorRect:rect displayArea:displayArea permittedArrowDirections:arrowDirections properties:props];
        ...
        if (animated) {
            ...
            if (self.animationType == KWEPopoverAnimationTypeSlide) {
                ...
                animate(firstAnimationDuration, ^{
                    ...
                    if (self.transitionBlock) {
                        self.transitionBlock(KWEPopoverTransitionTypePresent, animated);
                    }
...
- (void)repositionPopoverFromRect:(CGRect)rect
    ...
    if ([self isPopoverVisible] && !self.isDismissing) {
        ...
        KWEPopoverContainerView *containerView = self.containerView;
        ...
        void (^animationBlock)(void) = ^(void) {
            ...
            if (self.transitionBlock) {
                self.transitionBlock(KWEPopoverTransitionTypeReposition, animated);
            }
        };
...
#pragma mark -
#pragma mark KWETouchableViewDelegate implementation
...
@implementation KWEPopoverController(Private)
...
- (void)dismissPopoverAnimated:(BOOL)animated userInitiated:(BOOL)userInitiated completion:(KWEPopoverCompletionBlock)completion {
    ...
    if (self.containerView && !self.isDismissing && !self.isPresenting) {
        ...
        void (^animationCompletionBlock)(BOOL finished) = ^(BOOL finished) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:KWEPopoverControllerDidDismissNotification object:self];
            ...
            if (self.animationType == KWEPopoverAnimationTypeSlide) {
                ...
                animate(firstAnimationDuration, ^{
                    ...
                    animate(secondAnimationDuration, ^{
                        ...
                        if (self.transitionBlock) {
                            self.transitionBlock(KWEPopoverTransitionTypeDismiss, animated);
                        }
...
- (CGRect)displayAreaForView:(UIView *)theView {
    ...
    if (!inViewHierarchy) {
        NSException *ex = [NSException exceptionWithName:@"KWEInvalidViewHierarchyException" reason:@"The supplied view to present the popover from is not in the same view hierarchy as the parent view for the popover" userInfo:nil];
        @throw ex;
    }
    ...
    } else if ([theView conformsToProtocol:@protocol(KWEPopoverParentView)] && [theView respondsToSelector:@selector(displayAreaForPopover)]) {
        displayArea = [(id <KWEPopoverParentView>)theView displayAreaForPopover];
    } else {
